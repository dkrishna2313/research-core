...
class TestComputeOverallStatus:
    ...
    def test_critical_gap_is_fail(self) -> None:
        assert compute_overall_status(0.8, [_make_gap(GapSeverity.CRITICAL)], 1) == DiagnosticStatus.FAIL
    ...
    def test_non_critical_gaps_present_is_indeterminate(self) -> None:
        assert compute_overall_status(0.8, [_make_gap(GapSeverity.HIGH)], 1) == DiagnosticStatus.INDETERMINATE

    def test_medium_gap_is_indeterminate(self) -> None:
        assert compute_overall_status(0.9, [_make_gap(GapSeverity.MEDIUM)], 1) == DiagnosticStatus.INDETERMINATE
