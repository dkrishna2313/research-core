"""
Fixture provider composition for the research-core CLI.

build_fixture_engine() returns a fully configured ResearchEngine that uses
deterministic in-memory providers and the real RC4-RC7 pipeline components.
It requires no network, no Knowledge Layer runtime, and no external configuration.

All fixture data is hard-coded and deterministic. Output is stable across runs
when a fixed clock is supplied.
"""
...
def build_fixture_engine(
    *,
    ...
    """Build a ResearchEngine backed entirely by deterministic in-memory fixtures.
